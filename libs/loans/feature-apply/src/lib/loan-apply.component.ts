...
} from "@banking/loans/data-access";
...
@Component({
  ...
  template: `
    <div class="apply-container">
      ...
      @if (facade.applicationResult(); as result) {
        <div class="success-banner">
          ...
          <p>
            Estimated APR: <strong>{{ result.estimatedApr }}%</strong>
          </p>
          <p>
            Estimated Monthly Repayment:
            <strong>{{
              formatAmount(result.estimatedMonthlyRepayment)
            }}</strong>
          </p>
          <p>
            Decision expected by:
            <strong>{{ result.decisionExpectedBy }}</strong>
          </p>
        </div>
      }

      @if (!facade.applicationResult()) {
        <form [formGroup]="loanForm" (ngSubmit)="onSubmit()">
          ...
          <div class="form-field">
            ...
            <input
              ...
              step="500"
            />
            @if (loanForm.get("requestedAmount")?.value) {
              <span class="hint">
