...
export class LoansFacade {
  ...
  private readonly _applicationResult = signal<LoanApplicationResponse | null>(
    null,
  );
  ...
  loadLoans(customerId: string): void {
    ...
    this.loansApi.getLoans(customerId).subscribe({
      ...
      error: () => {
        this._error.set("Failed to load loans.");
        this._isLoading.set(false);
      },
    });
  }
  ...
  applyForLoan(
    ...
    return this.loansApi.applyForLoan(request).pipe(
      tap({
        next: (result) => {
          this._applicationResult.set(result);
          this._isLoading.set(false);
        },
        error: () => {
          this._error.set("Loan application failed.");
          this._isLoading.set(false);
        },
      }),
    );
