...
import {
  ButtonComponent,
  BadgeComponent,
} from "@banking/shared/ui-design-system";
...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, ButtonComponent, BadgeComponent],
  template: `
    <div class="onboarding-container">
      <h2>Account Onboarding</h2>
      <p class="subtitle">
        We need to verify your identity to open your account. This is a legal
        requirement.
      </p>

      @if (facade.onboardingResult(); as result) {
        <div class="result-card">
          <banking-shared-badge variant="info">{{
            result.kycStatus
          }}</banking-shared-badge>
          <h3>Application Received</h3>
          <p>
            Reference: <strong>{{ result.applicationId }}</strong>
          </p>
          <p>{{ result.nextStep }}</p>
        </div>
      }

      @if (!facade.onboardingResult()) {
        <form [formGroup]="onboardingForm" (ngSubmit)="onSubmit()">
          <fieldset>
            ...
            <div class="form-field">
              ...
              @if (onboardingForm.get("dateOfBirth")?.errors?.["notAdult"]) {
                <span class="error"> You must be 18 or older to apply. </span>
              }
            </div>
            <div class="form-field">
              ...
              />
              @if (
                onboardingForm.get("nationalInsuranceNumber")?.errors?.[
                  "invalidNi"
                ]
              ) {
                <span class="error"> Please enter a valid NI number. </span>
              }
            </div>
          </fieldset>
          <fieldset>
    ...
    this.onboardingForm = this.fb.group({
      ...
      dateOfBirth: [
        ...
        [
          Validators.required,
          (c: any) =>
            c.value && !isAdult(c.value) ? { notAdult: true } : null,
        ],
      ],
      nationalInsuranceNumber: [
        ...
        [
          Validators.required,
          (c: any) =>
            c.value && !isValidNiNumber(c.value) ? { invalidNi: true } : null,
        ],
      ],
