...
import { isValidSortCode, isValidAccountNumber, isValidTransferAmount } from '@banking/shared/util-validators';
...
@Component({
    ...
    <div class="transfer-container">
      ...
      <form [formGroup]="transferForm" (ngSubmit)="onSubmit()">
        <div class="form-field">
          ...
          <input id="toName" formControlName="toName" placeholder="John Smith" />
        </div>

        <div class="form-field">
          <label for="toSortCode">Sort Code</label>
          <input id="toSortCode" formControlName="toSortCode" placeholder="12-34-56" />
          <span class="hint">Format: XX-XX-XX</span>
        </div>

        <div class="form-field">
          <label for="toAccountNumber">Account Number</label>
          <input id="toAccountNumber" formControlName="toAccountNumber" placeholder="12345678" />
        </div>

        <div class="form-field">
          <label for="amount">Amount (£)</label>
          <input id="amount" type="number" formControlName="amount" placeholder="0.00" min="0.01" step="0.01" />
          <span class="preview" *ngIf="transferForm.get('amount')?.value">
        ...
        <div class="form-field">
          ...
          <input id="reference" formControlName="reference" placeholder="Invoice 1234" maxlength="18" />
        </div>
  ...
  `,
  styles: [`
    .transfer-container { max-width: 480px; margin: 0 auto; }
    ...
    input { padding: 0.625rem; border: 1.5px solid #d1d5db; border-radius: 6px; font-size: 1rem; }
    input:focus { outline: none; border-color: #1a1a2e; }
    .hint { font-size: 0.75rem; color: #9ca3af; }
    .preview { font-size: 0.875rem; color: #065f46; font-weight: 600; }
...
export class TransferComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.transferForm = this.fb.group({
      ...
      toSortCode: ['', [Validators.required, (c: any) => isValidSortCode(c.value) ? null : { invalidSortCode: true }]],
      toAccountNumber: ['', [Validators.required, (c: any) => isValidAccountNumber(c.value) ? null : { invalidAccountNumber: true }]],
      amount: [null, [Validators.required, (c: any) => isValidTransferAmount(c.value) ? null : { invalidAmount: true }]],
