...
import { formatCurrency, formatDate, maskAccountNumber } from '@banking/shared/util-formatters';
...
@Component({
  ...
  template: `
    <div class="history-container">
      ...
      <div *ngIf="facade.isLoading()" class="loading">Loading transactions...</div>

      <div *ngIf="!facade.isLoading() && facade.transactions().length === 0" class="empty">
        No transactions found.
      </div>

      <ul class="transaction-list" *ngIf="facade.transactions().length > 0">
        <li *ngFor="let txn of facade.transactions()" class="transaction-item">
          <div class="txn-left">
            <span class="txn-name">{{ txn.toName }}</span>
            <span class="txn-account">{{ maskAccount(txn.toAccountNumber) }}</span>
            <span class="txn-date">{{ formatDate(txn.createdAt) }}</span>
          </div>
          <div class="txn-right">
            <span class="txn-amount">{{ formatAmount(txn.amount, txn.currencyCode) }}</span>
  ...
  `,
  styles: [`
    .history-container { max-width: 720px; }
    .transaction-list { list-style: none; padding: 0; margin: 0; }
    .transaction-item { display: flex; justify-content: space-between; align-items: center; padding: 1rem; border-bottom: 1px solid #e5e7eb; }
    .txn-left { display: flex; flex-direction: column; gap: 0.125rem; }
    .txn-name { font-weight: 600; }
    .txn-account, .txn-date { font-size: 0.8rem; color: #6b7280; }
